  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(201).json({
        ...
        diagnosis: plan.diagnoza,
        description: plan.descriere,
        isActive: plan.activ,
        createdAt: plan.data_creare,
...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    res.json(result.rows.map((tp: any) => ({
      ...
      diagnosis: tp.diagnoza,
      description: tp.descriere,
      isActive: tp.activ,
      createdAt: tp.data_creare,
  ...
  try {
    ...
    res.json({
      ...
      medications: medications.rows.map((m: any) => ({
        ...
        endDate: m.end_date,
        instructions: m.instructiuni,
        medicationDetails: m.detalii_medicament,

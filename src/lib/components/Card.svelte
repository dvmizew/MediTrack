<script lang="ts">
	...
	let { title, value, sub = '', accent = 'text-gray-900 dark:text-slate-100', icon: Icon, iconColor = 'text-gray-500 dark:text-slate-300', iconBg = 'bg-gray-100 dark:bg-slate-800/40', ariaLabel }: Props = $props();

	const computedAriaLabel = $derived(ariaLabel ?? `${title}: ${value}${sub ? `, ${sub}` : ''}`);
</script>

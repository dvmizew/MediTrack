...
<div class={`p-4 md:p-5 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all duration-200 ${isTaken ? 'bg-green-50 dark:bg-green-900/20' : ''}`}>
	<div class="flex flex-col sm:flex-row justify-between items-start gap-3 md:gap-4">
		<div class="flex-1 min-w-0 w-full">
			...
			<div class="space-y-1">
				<p class="text-xs md:text-sm text-gray-600 dark:text-gray-400">💊 Doza: <span class="font-medium">{medication.cantitate}</span></p>
				<p class="text-xs md:text-sm text-gray-600 dark:text-gray-400">🔄 Frecvență: <span class="font-medium">{medication.frecventa}</span></p>
				{#if medication.instructiuni}
					<p class="text-xs md:text-sm text-gray-500 dark:text-gray-500 mt-2 italic line-clamp-2">{medication.instructiuni}</p>
				{/if}
			</div>
		</div>
